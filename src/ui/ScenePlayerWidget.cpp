...
#include "render/scenes/ConcertScene.h"
#include "render/scenes/CinemaScene.h"
#include "render/scenes/TechPlazaScene.h"
...
ScenePlayerWidget::ScenePlayerWidget(SessionManager *sessionManager, QWidget *parent)
    : QWidget(parent), m_sessionManager(sessionManager)
{
    ...
    m_sceneWidget = new OpenGLWidget(this);
    m_sceneWidget->setVideoDecoder(m_manager);
    m_sceneWidget->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);
    layout->addWidget(m_sceneWidget, 1);
    ...
    connect(m_exitBtn, &QPushButton::clicked, this, &ScenePlayerWidget::backToHome);
    ...
    connect(this, &ScenePlayerWidget::backToHome, this, [this]() {
    ...
    connect(m_controlPanel, &SceneControlPanel::prevRequested, this, [this]() {
        ...
        if (idx > 0) {
            ...
            if (sel) {
                unload();
                loadFile(sel->filePath, m_currentSceneType);
            }
        }
    });
    ...
    connect(m_controlPanel, &SceneControlPanel::showPlaylistRequested, this, [this]() {
        if (!m_playlistPopup) {
            ...
            connect(m_playlistPopup, &PlaylistPopup::fileSelected, this, [this](const QString &path, int index) {
                ...
                unload();
                loadFile(path, m_currentSceneType);
            });
        }
...
ScenePlayerWidget::~ScenePlayerWidget() {}

void ScenePlayerWidget::loadFile(const QString &filePath, SceneType sceneType) {
    ...
    if (m_isNetworkFile) {
        // ── Async open for network files: keep UI responsive ──
        int token = ++m_openToken;
        m_loadingOverlay->showWithDelay(0);
        QApplication::processEvents();

        std::thread([this, filePath, sceneType, token, manager = m_manager]() {
            bool ok = manager->open(filePath.toStdString());
            QMetaObject::invokeMethod(this, [this, ok, filePath, sceneType, token, manager]() {
                ...
                if (ok) {
                    ...
                    info.scene = sceneType;
                    m_controlPanel->updateMediaInfo(info);
                    m_playTimer->start(150);
                } else {
                    m_loadingOverlay->hideImmediately();
                    m_bufferingTimer->stop();
        ...
        // ── Local files: sync open (fast, no blocking) ──
...
void ScenePlayerWidget::createScene(SceneType type) {
    switch (type) {
        case SceneType::Cinema:
            m_sceneWidget->setScene(std::make_unique<CinemaScene>());
            break;
        case SceneType::TechPlaza:
            m_sceneWidget->setScene(std::make_unique<TechPlazaScene>());
            break;
        case SceneType::Concert:
        default:
            m_sceneWidget->setScene(std::make_unique<ConcertScene>());
            break;
    }
}

void ScenePlayerWidget::unload() {
    m_playTimer->stop();
    ++m_openToken;                  // 丢弃任何 pending 回调
    m_manager->cancelOpen();        // 中断可能正在进行的网络 I/O
    m_manager->close();
    m_loadingOverlay->hideImmediately();
    m_bufferingTimer->stop();
    if (m_visPopup) {
...
void ScenePlayerWidget::togglePanel() {
    m_panelVisible = !m_panelVisible;
    m_controlPanel->setVisible(m_panelVisible);
    if (m_panelVisible) updateControls();
}

void ScenePlayerWidget::updateControls() {
    int idx = m_sessionManager->selectedIndex();
    m_hasPrevious = idx > 0;
    m_hasNext = idx >= 0 && idx < m_sessionManager->files().size() - 1;
    m_controlPanel->setPrevNextEnabled(m_hasPrevious, m_hasNext);
...
void ScenePlayerWidget::resizeEvent(QResizeEvent *event) {
...
void ScenePlayerWidget::keyPressEvent(QKeyEvent *event) {
...
void ScenePlayerWidget::mousePressEvent(QMouseEvent *event) {
